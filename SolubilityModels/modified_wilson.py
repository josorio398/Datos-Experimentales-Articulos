...
class dataset:
    ...
    def experimental_values(self,scale = 0,download_format = "None"):
        ...
        -------------------------------------------------------------------------------
        Method to show and download in different formats the datafram experimental values 
    ...
    def molar_fractions(self,mf):
        ...
        if mf == "x1":
            df_mf = pd.DataFrame({"x1":df["x1"]})
        if mf == "x2":
            df_mf = pd.DataFrame({"x2":df["x2"]})
        if mf == "x3":
